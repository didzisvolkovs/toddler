<?php
class ControllerProductRetail extends Controller {
	public function index() {
		$this->load->language('product/retail');
		...
		$data['breadcrumbs'][] = array(
			...
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('product/retail')
			);
  ...
  // echo count($options);
			...
			$data['sorts'][] = array(
				...
				'href'  => $this->url->link('product/retail',  '&sort=p.sort_order&order=ASC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_name_asc'),
				'value' => 'pd.name-ASC',
				'href'  => $this->url->link('product/retail',  '&sort=pd.name&order=ASC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_name_desc'),
				'value' => 'pd.name-DESC',
				'href'  => $this->url->link('product/retail',  '&sort=pd.name&order=DESC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_price_asc'),
				'value' => 'p.price-ASC',
				'href'  => $this->url->link('product/retail',  '&sort=p.price&order=ASC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_price_desc'),
				'value' => 'p.price-DESC',
				'href'  => $this->url->link('product/retail',  '&sort=p.price&order=DESC' . $url)
			);
			...
			if ($this->config->get('config_review_status')) {
				$data['sorts'][] = array(
					...
					'value' => 'rating-DESC',
					'href'  => $this->url->link('product/retail',  '&sort=rating&order=DESC' . $url)
				);

				$data['sorts'][] = array(
					'text'  => $this->language->get('text_rating_asc'),
					'value' => 'rating-ASC',
					'href'  => $this->url->link('product/retail',  '&sort=rating&order=ASC' . $url)
				);
			}
			...
			$data['sorts'][] = array(
				...
				'value' => 'p.model-ASC',
				'href'  => $this->url->link('product/retail',  '&sort=p.model&order=ASC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_model_desc'),
				'value' => 'p.model-DESC',
				'href'  => $this->url->link('product/retail',  '&sort=p.model&order=DESC' . $url)
			);
			...
			foreach($limits as $value) {
				$data['limits'][] = array(
					'text'  => $value,
					'value' => $value,
					'href'  => $this->url->link('product/retail',  $url . '&limit=' . $value)
				);
			}
			...
			$pagination->url = $this->url->link('product/retail',  $url . '&page={page}');
			...
			if ($page == 1) {
			    $this->document->addLink($this->url->link('product/retail'), 'canonical');
			} else {
				$this->document->addLink($this->url->link('product/retail', '&page='. $page), 'canonical');
			}

			if ($page > 1) {
			    $this->document->addLink($this->url->link('product/retail', (($page - 2) ? '&page='. ($page - 1) : '')), 'prev');
			}

			if ($limit && ceil($product_total / $limit) > $page) {
			    $this->document->addLink($this->url->link('product/retail', '&page='. ($page + 1)), 'next');
			}
